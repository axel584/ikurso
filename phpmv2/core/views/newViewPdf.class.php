...
// $Id: ViewPdf.class.php 76 2006-09-14 07:08:51Z matthieu_ $
//define ("INCLUDE_PATH", "../..");
	...
	function display() {
		...
		*/
		...
		if ($idSite <= 0) {
			...
			$confPf = new PdfConfigDb($idSite);

			$paramShowPdf = $confPf->getPdf(1);
			...
			foreach ($paramShowPdf as $key => $info) {
				if ($key == "PG0") {
					$pdf->titrePage1 = "summary_title";
					$pdf->AddPage();
					$firstPageOk = true;
					...
					if (isset ($confShowPdf[$key]["PAG"])) {
						...
						$pdf->SetX(50);
						$pdf->Cell(80, PDF_DEFAULT_ROW_HEIGHT, $pdf->pmvTranslate($confShowPdf[$key]["TIT"]), "0", "", "L", 0, $pdfLink[$key]);
						$pdf->Ln();
					}
			}
			...
			foreach ($paramShowPdf as $key => $info) {
				if ($key == "PG0") {
					...
					if (isset ($confShowPdf[$key]["PAG"])) {
						$pdf->titrePage1 = $confShowPdf[$key]["TIT"];
						$pdf->AddPage();
						$firstPageOk = true;
						...
						if (($confShowPdf[$key]["INT"] == "true") && ($confShowPdf[$key]["ALL"] == "true")) {
							...
							if ($confShowPdf[$key]["INT"] == "true") {
								$pdf-> $confShowPdf[$key]["FCT"] ($info[0]);
							} else
								if ($confShowPdf[$key]["ALL"] == "true") {
									$pdf-> $confShowPdf[$key]["FCT"] ($info[1]);
								} else {
